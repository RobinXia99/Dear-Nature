...
class MapService {
    ...
    func getPlaces(map: UserMap, completion: @escaping (_ places: [Place]) -> Void) {
        ...
        db.collection("places").document(currentUser.uid).collection("userPlaces").whereField("mapID", isEqualTo: map.id!).addSnapshotListener { snapshot, err in
            guard let documents = snapshot?.documents else {
                print("Error fetching documents: \(err!)")
                return
            }
    ...
    func createPlace(map: UserMap, place: Place, completion: @escaping (_ newPlace: Place) -> Void) {
        ...
        do {
            _ = try db.collection("places").document(currentUser.uid).collection("userPlaces").addDocument(from: place).getDocument() { snapshot, err in
                if err != nil {
                    print("error creating place 1")
                } else {
                    if let document = snapshot {
                        self.db.collection("maps").document(currentUser.uid).collection("userMaps").document(map.id!).updateData(["places": FieldValue.arrayUnion([document.documentID])])
                    }
        ...
        if placeImage != nil {
            storage.saveImageToStorage(image: placeImage) { ref, error in
                ref.downloadURL { url, error in
                    ...
                    self.db.collection("places").document(currentUser.uid).collection("userPlaces").document(place.id!)
                        .updateData(["name": placeName, "placeImage": url.absoluteString, "markerSymbol": markerSymbol, "placeInfo": placeInfo]) { err in
                        if let error = err {
                            print("error updating map: \(error)")
                            return
                        } else {
    ...
    func deletePlace(place: Place, map: UserMap, completion: @escaping (_ success: Bool) -> Void) {
        ...
        self.db.collection("places").document(currentUser.uid).collection("userPlaces").document(place.id!).delete() { err in
            if let err = err {
                print("error deleting place \(err)" )
            } else {
                
                self.db.collection("maps").document(currentUser.uid).collection("userMaps").document(map.id!).updateData(["places": FieldValue.arrayRemove([place.id!])])
